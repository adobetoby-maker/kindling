# Living book map — Book 2

Working title: **"The Ledger"** (`POSSIBLE`, not structural) — echoes Nell
Quill's book (Book 1), Meridian's own bookkeeping culture, and the archive's
"released to bearer" line with no name attached.

Status: `POSSIBLE`. Developed jointly with the Tallow Gate/Undern authoring
...
- **Repeating reader pleasures:** the specific mechanical thrill of a new
  technique nobody has ever had (Stride's Guard shape — see `CANON.md`) being found,
...
- **Ending promise:** the team's whole toolkit is measurably wider than it
  was on the first page — Edge+Stride-as-Guard trained and reliable for at
...
- **Meridian's Kindled registration and labor system** — the book's real
  antagonist, and it should be staffed by people who are mostly right about
  their own reasoning, per house discipline (see Book 1's Nell Quill/Warden
  Braid pattern). Working default: a Director's-office official — name open,
...
- **Cinder Company's remnant** — off-page all book, per the shared agreement
  ...
  a direct confrontation this book. Reserved for Book 3's larger canvas.
- **The disk / Annice Fell's archive** — not antagonistic, but a standing
  pressure: the "released to bearer" mystery is live, and Meridian's own
...
| Dessa's confirmed Ember rank, thirty-foot sensing | Ch. 18, 26 | She's the one who discovers Guard by accident — proof her door was never only support | `POSSIBLE` |
...
trained, repeatable technique (Guard) that at least two teammates can use
...
- ~~Guard's exact name and mechanism~~ — **resolved.** Converged with the
  other authoring session: Guard is Stride's second *shape* (structural, the
  same strut hardware held rigid instead of loaded for motion), explicitly
  not Ward and not a third door, keeping the domain-separation rule intact.
  Both editions now share the same name and the same mechanism.
...
- A direct Cinder Company confrontation as Book 2's climax — retired in favor
  of holding them off-page, per the shared agreement with the other edition,
  so Book 3 has a genuinely larger canvas to escalate into rather than
  re-spending Book 1's antagonist too early.
